...
// Define new type for monitor log data
interface MonitorLogCatch {
  id: number
  monitor_name: string
...
const columnHelper = createColumnHelper<MonitorLogCatch>()

const MonitorLogCatchTable = () => {
  const [rowSelection, setRowSelection] = useState({})
  const [data, setData] = useState<MonitorLogCatch[]>([])
  const [filteredData, setFilteredData] = useState<MonitorLogCatch[]>([])
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/api/pages/monitor_history')
        const result = await response.json()

        // Add role field to each record if not present
        const processedData = result.map((item: MonitorLogCatch) => ({
          ...item,
          role: item.role || 'monitor' // Default role if not provided by API
        }))
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('monitor_name', {
        header: 'User Name',
        cell: ({ row }) => (
      ...
      columnHelper.accessor('landing_date', {
        header: 'Landing Date',
        cell: ({ row }) => <Typography>{new Date(row.original.landing_date).toLocaleDateString()}</Typography>
      }),
      columnHelper.accessor('role', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-2'>
            <Icon
              className={classnames('text-[22px]', userRoleObj[row.original.role]?.icon || userRoleObj['monitor'].icon)}
              sx={{
  ...
  return (
    <Card>
      <CardHeader title='Monitor Log Catch Data' className='pbe-4' />
      <Divider />
...
export default MonitorLogCatchTable
