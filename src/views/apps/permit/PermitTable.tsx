...
type UserStatusApplicationType = {
  [key: string]: ThemeColor
}
...
const PermitTable = ({ tableData }: { tableData?: UsersType[] }) => {
  // States
      ...
      columnHelper.accessor('permit', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-2'>
            <Icon
              className={userPermitObj[row.original.permit].icon}
              sx={{
                color: `var(--mui-palette-${userPermitObj[row.original.permit].color}-main)`,
                fontSize: '1.375rem'
              }}
            />
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-3'>
            <Chip
              ...
              size='small'
              color={userStatusApplicationObj[row.original.statusapplications]}
              className='capitalize'
            />
          </div>
        )
  ...
  useEffect(() => {
    const filteredData = data?.filter(user => {
      if (permit && user.permit !== permit) return false

      return true
    })

    setFilteredData(filteredData)
  }, [permit, data, setFilteredData])
      ...
      <CardContent className='flex justify-between flex-col items-start sm:flex-row sm:items-center max-sm:gap-4'>
        ...
        <div className='flex flex-col !items-start max-sm:is-full sm:flex-row sm:items-center gap-4'>
          ...
          <FormControl size='small' className='max-sm:is-full'>
            <InputLabel id='permits-app-permit-select-label'>Select Permit</InputLabel>
            <Select
              value={permit}
              onChange={e => setPermit(e.target.value)}
              label='Select Permit'
              id='permit-app-permit-select'
              labelId='permits-app-permit-select-label'
